## `feedback_system.py` – Decentralized User Feedback Contract
...
- **`delete_feedback() -> None`**  
  - Removes the caller’s rating and comment from both `TreeMap`s.  
...
### Deployment & Testing
1. Copy the code into a new file `feedback_system.py` in [GenLayer Studio](https://studio.genlayer.com).
2. Click **Deploy** – the constructor takes no arguments.
3. Use the **Write** panel to call `submit_feedback` with a rating and comment.
4. Switch between different test accounts to simulate multiple users.
5. Read the average rating or list all participants via the **Read** panel.
